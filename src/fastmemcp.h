...
static INLINE bool Is_supportAVX()
{
	...
	else
	{
		...
		return ((cpuidInfoArr[2] >> 28) & 1);
	}

__not_support:
	return 0;
	//MessageBoxW(NULL, L"当前CPU不支持AVX！\nAVX Instruction Set Not Support!", L"Fatal Error", 0x10);
	//ExitProcess(0);
}

/*
static bool Is_supportAVX2()
{
	int32_t info = CPUID_07_EBX;
	info = (info >> 5) & 1;
	if (info)
	{
		return 1;
	...
	return 0;
	//MessageBoxW(NULL, L"当前CPU不支持AVX2！\nAVX2 Instruction Set Not Support!", L"Fatal Error", 0x10);
	//ExitProcess(0);
}

static bool Is_supportAVX512F()
{
	int32_t info = CPUID_07_EBX;
	info = (info >> 16) & 1;
	if (info)
	...
	return 0;
	//MessageBoxW(NULL, L"当前CPU不支持AVX512F！\nAVX512F Instruction Set Not Support!", L"Fatal Error", 0x10);
	//ExitProcess(0);
}
*/
